...
# PIN `longnav` TO THIS CHECKOUT, BEFORE ANY `longnav` IMPORT BELOW.
# There is no `src/longnav/__init__.py`, so `longnav` is a NAMESPACE package: a bare import
# resolves to whichever clone is first on `sys.path`, and on this machine that is
# `/Projects/Codex_Projects/spatial_training/src/longnav`. The symptom is not an obvious one
# -- `tests/rollout` fails to collect with "No module named 'longnav.env.replay'" for a file
# that plainly exists in this tree, because the OTHER clone answered. Without this, the whole
# suite silently tests a different checkout.
_ROOT = Path(__file__).resolve().parents[1]
for _p in (str(_ROOT), str(_ROOT / "src")):
    if _p not in sys.path:
        sys.path.insert(0, _p)

# And again through the ENVIRONMENT, because `sys.path` is not inherited by child processes.
# Ray actors (`ReplayEnvActor`, the VLM workers) are separate processes that re-import
# `longnav` from scratch, so without this they resolve to the foreign clone even though the
# parent process is correctly pinned -- which presents as `ActorDiedError` wrapping a
# `ModuleNotFoundError` for a module that plainly exists in this tree.
_existing = os.environ.get("PYTHONPATH", "")
_want = str(_ROOT / "src")
if _want not in _existing.split(os.pathsep):
    os.environ["PYTHONPATH"] = (_want + os.pathsep + _existing) if _existing else _want
...
def pytest_addoption(parser):
    """Kept from the pre-merge conftest: `tests/test_vector_rollout.py` reads this.

    `continuous_action` replaced this file wholesale and defines no options, so taking it
    verbatim would break that test with an unrecognised-argument error.
    """
    parser.addoption(
        "--ckpt",
        action="store",
